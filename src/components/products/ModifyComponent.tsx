...
import { ProductReadComponentProps } from "types";
...
const ModifyComponent = ({ pno }:ProductReadComponentProps) => {
  // 결과 모달
  ...
  const uplodaRef = useRef<HTMLInputElement>(null);
  ...
  const handleChangeProduct = (e: React.ChangeEvent<HTMLSelectElement> | React.ChangeEvent<HTMLInputElement> | React.ChangeEvent<HTMLTextAreaElement>) => {
    setProduct({
      ...product,
  ...
  const modMutation = useMutation((product: FormData) => putOne(pno, product));
  ...
  const delMutation = useMutation((pno: number) => deleteOne(pno));
